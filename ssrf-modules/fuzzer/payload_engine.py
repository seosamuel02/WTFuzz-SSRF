...
def main():
    ...
    print("=== 지능형 페이로드 생성 엔진 테스트 ===")

    # 초기 페이로드 생성
    initial_payloads = engine.generate_initial_payloads(target_info)
    print(f"\n초기 페이로드 생성: {len(initial_payloads)}개")

    for i, payload in enumerate(initial_payloads[:5], 1):
        print(f"  {i}. [{payload['priority']}] {payload['payload']}")
